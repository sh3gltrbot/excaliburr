...
exports.run = (client, message, args) => {
    ...
    if (message.mentions.users.size < 2) return message.channel.send(new Discord.RichEmbed().setColor('RANDOM').setTitle('Efkarlı.JPG').setDescription(message.author.tag + ', kullanım: x!efkar-vs <@kullanıcı> <@kullanıcı> .').setFooter('', client.user.avatarURL).setTimestamp());
